...
void jogo(const char* nome, int vida, int atributo_principal, int forca, int constituicao, int destreza, int inteligencia, int profissao, int alinhamento) {
    ...
    printf("Após um longo dia de estudos intensos no IDP, você começa a organizar seus materiais para finalmente ir embora.\n");
    printf("Absorvido pela concentração, não percebeu o tempo passar — agora, os corredores estão vazios, as luzes apagadas e a faculdade completamente silenciosa.\n");
    printf("Ao alcançar o estacionamento, o som dos seus passos ecoa no vazio. Você se aproxima do seu Peugeot 206, mas ao tocar o bolso, percebe algo estranho: a chave não está lá.\n");
    getchar();
    printf("Com o coração acelerado e apenas a luz fraca de uma lanterna em mãos, você se dá conta de que terá que voltar, sozinho, para dentro do prédio escuro.\n");
    printf("Em meio à escuridão e ao silêncio absoluto, sua única missão é clara: encontrar a chave... e sair dali antes que algo — ou alguém — encontre você primeiro.\n");
...
void jogo_facil(int escolha_mapa, const char* nome, int vida, int atributo_principal, int forca, int constituicao, int destreza, int inteligencia, int profissao, int alinhamento, const char* atributo_principal_escrito, const char* profissao_escrito, const char* alinhamento_escrito)
{
    ...
    while(vida > 0 && ir_para_boss == 0) {
        ...
        if(mapa_1[n][m] == '-') {
            printf("\nVocê acionou uma armadilha. Parte da sua energia foi drenada.\n");
        }

        if(mapa_1[n][m] == 'H') {
            printf("\nVocê encontrou um cookie da Johns. Ao comer, sente-se levemente revigorado. (+20 HP)\n");
        }

        if(mapa_1[n][m] == 'V') {
            printf("\nUma bateria potente. Sua lanterna agora ilumina muito mais longe por um tempo.\n");
        }

        if(mapa_1[n][m] == 'P') {
            printf("\nVocê bebe um energético. Seu atributo principal foi temporariamente fortalecido.\n");
        }

        if(mapa_1[n][m] == 'C') {
            ir_para_boss = 1;
            printf("\n\nVocê encontra Nilson, segurando as chaves do seu Peugeot 206.\n");
            getchar();
        }

        if(vida <= 0) {
            printf("\nVocê caiu exausto no chão, sem forças para continuar. A chave jamais foi encontrada.\n");
            getchar();
        }
    }
...
void jokenpo(int alinhamento, const char* nome, int *vida) {
    ...
    printf("Seu destino está selado... a menos que consiga me derrotar em uma partida de JOKENPÔ.\n");
    getchar();
...
void charada(int vida) {
    ...
    printf("Você realmente achou que seria tão fácil? Agora, se quiser continuar, terá que acertar três charadas que vou te fazer.\n\n");
    getchar();
    getchar();
...
/*REPOSTAS DAS CHARADAS*/
    ...
    if (vida > 0) {  // VITÓRIA
        printf("\n\nVocê resolve a última charada com precisão. O silêncio se instala por um momento.\n");
        getchar();
        getchar();
        printf("Nilson o encara em silêncio, claramente impressionado.\n");
        printf("Sem dizer uma palavra, ele entrega lentamente a chave do seu Peugeot 206.\n");
        getchar();
        printf("Você a segura firme, respira fundo e segue pelo corredor escuro em direção à saída.\n");
        printf("Do lado de fora, a noite ainda cobre a cidade... mas você venceu.\n");
        printf("Você escapou.\n");
        getchar();
        getchar();
    } else {  // DERROTA
        printf("\n\nVocê hesita... erra a última resposta. O olhar de Nilson se torna frio.\n");
        getchar();
        getchar();
        printf("— Não é o bastante — ele diz, antes de desaparecer na escuridão com a chave.\n");
        printf("Preso nas sombras da faculdade, você percebe que a saída não virá tão cedo.\n");
        printf("Talvez... nem venha.\n");
        printf("Você falhou.\n");
        getchar();
        getchar();
    }
